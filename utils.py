...
'''def recall_at_k(y_true_seq, y_pred_seq, k):
    """
    计算Recall@K。
    :param y_true_seq: 真实的POI序列，形状为 (seq_len,)
    :param y_pred_seq: 预测的POI序列分数，形状为 (seq_len, num_poi)
    :param k: 选择的K值
    :return: 返回Recall@K的值
    """
    hits = 0
...
def recall_at_k(y_true, y_pred, k):
    """
    计算Recall@K。
    :param y_true: 真实的POI索引，类型为整数
    :param y_pred: 预测的POI分数列表，形状为 (num_poi,)
    :param k: 选择的K值
    :return: 返回Recall@K的值
    """
    # 获取预测分数中前K个POI的索引
